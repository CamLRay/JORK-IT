  ...
  return(
    <nav className="fixed bottom-0 left-0 bg-zinc-900 text-stone-200 min-w-full grid grid-cols-5 gap-2 place-items-center border-t-1 border-t-zinc-400">
      {menu.map((option, index) => (
        <Link key={index} href={"/" + option.name} className="capitalize grid grid-rows-1 place-items-center py-2 text-xs" ><Icon icon={option.icon[1]!} className="text-3xl text-white"/>{option.name}</Link>
      ))}
    </nav>
  );
}